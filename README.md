...
- [poetry-ranking](#poetry-ranking)
  - [Содержание](#содержание)
    ...
    - [Метрика](#метрика)
    - [Токенезация](#токенезация)
    - [Варианты моделей](#варианты-моделей)
      - [Вариант 1. Решение через задачу регрессии](#вариант-1-решение-через-задачу-регрессии)
        - [Преимущества](#преимущества)
        - [Недостатки](#недостатки)
      - [Вариант 2. Решение через задачу классификации](#вариант-2-решение-через-задачу-классификации)
      - [Вариант 3. Обучение с triplet loss](#вариант-3-обучение-с-triplet-loss)
        - [Преимущества](#преимущества-1)
        - [Недостатки](#недостатки-1)
      - [Вариант 4. bi-encoder + cross-endoder](#вариант-4-bi-encoder--cross-endoder)
      - [Доп. улучшения](#доп-улучшения)
    - [Вывод по результатам](#вывод-по-результатам)
    - [Источники](#источники)
...
Эксперименты, которые успел провести, показали, что в целом что-то у модели получается выучивать, но кажется не особо хорошо.

#### Вариант 2. Решение через задачу классификации

Ставим всему, где рейтинг 0 или около, класс 0, остальному ставим класс 1, рейтинги используем в качестве весов. На выходе получаем "вероятность" (скор) принадлежности классу 1 и чем она выше, тем потенциально более хорошее стихотворение.

#### Вариант 3. Обучение с triplet loss
...
#### Вариант 4. bi-encoder + cross-endoder
...
Реализация по ссылке предполагает поиск ответов на вопросов и ранжирование их по релевантности запросу, в случае стихов мои эксперименты показали, что оно по семантике также неплохо находит стихи, схожие по теме с апросом, который мы указали. Кажется, что можно попробовать адапатировать идею, чтобы вместо запроса был какой-то "хороший" стих и поиск шел бы не только по семантике, но и по тому, насколько хорошие стихи:
- первым этапом можно выделять группы (топ-к стихов, то что после топ-к и так далее);
- далее ранжировать уже в рамках полученной группы (если необходимо).
...
По итогу можно сказать, что данные достачно шумные, поскольку в них есть большое количество крайне странных "стихов", много нулевых рейтингов в связке с тем, что не совсем понятно как этот рейтинг формируется. Всё это ведёт к тому, что нужно очень тщательно продумывать предобработку, очистку данных. Мои эксперименты на текущий момент не показали заметно хороших результатов, но и все идеи я пока попробовать не успел.
...
- https://arxiv.org/pdf/2205.08847
- https://arxiv.org/pdf/2307.01387
